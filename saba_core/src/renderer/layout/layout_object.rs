...
/*
split_text関数は、ウィンドウの大きさによってテキストを指定された幅内に収まるように単語の途中で折り返すことなく、
スペースで区切られた部分ごとに分割する処理を行っています。
*/
...
impl LayoutObject {
    ...
    pub fn update_kind(&mut self) {
        match self.node_kind() {
            NodeKind::Document => panic!("should not create a layout object for Document node"),
            NodeKind::Element(_) => {
    ...
    pub fn paint(&mut self) -> Vec<DisplayItem> {
        ...
        match self.kind {
            ...
            LayoutObjectKind::Inline => {
                // 本書のブラウザでは、インライン要素自体は描画しない
                // imgタグなどのインライン要素があればここで描画処理を追加する
            }
            LayoutObjectKind::Text => {
                if let NodeKind::Text(t) = self.node_kind() {
                    let mut v = vec![];
