...
impl Url {
	...
	// URLを解析する
	// &mut selfは、メソッドが可変な参照経由でオブジェクト自体にアクセスすることを示します。
	...
	fn extract_host(&self) -> String {
		let url_parts: Vec<&str> = self
			.url
			.trim_start_matches("http://") // http://を削除
			.splitn(2, "/") // 最初のスラッシュまでを分割
			.collect();

		if let Some(index) = url_parts[0].find(':') { // ポート番号がある場合
	...
	fn extract_port(&self) -> String {
		...
		if let Some(index) = url_parts[0].find(':') {
			url_parts[0][index + 1..].to_string() // コロンの次から始まるポート番号を取得
		} else {
			"80".to_string() // デフォルトのポート番号
		}
	}
	...
	fn extract_path(&self) -> String {
		...
		if url_parts.len() < 2 { // パスがない場合
			return "".to_string();
		}

		let path_and_searchpart: Vec<&str> = url_parts[1].splitn(2, "?").collect(); // クエリパラメータの分割
