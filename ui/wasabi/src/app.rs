...
impl WasabiUI {
    ...
    fn handle_mouse_input(&mut self) -> Result<(), Error> {
        if let Some(MouseEvent {
            ...
            if button.l() || button.c() || button.r() {
                ...
                // ウィンドウの外をクリックされた時はなにもしない
                if relative_pos.0 < 0 || relative_pos.0 > WINDOW_WIDTH || relative_pos.1 < 0 || relative_pos.1 > WINDOW_HEIGHT {
                    println!("button clicked OUTSIDE window: {button:?} at position: {position:?}");
                    return Ok(());
                }

                // ツールバーの範囲をクリックされたとき、InputModeをEditingに変更する
                if relative_pos.1 < TOOLBAR_HEIGHT + TITLE_BAR_HEIGHT && relative_pos.1 >= TITLE_BAR_HEIGHT {
                    self.clear_address_bar()?;
                    self.input_url = String::new();
                    self.input_mode = InputMode::Editing;
                    println!("button clicked in toolbar: {button:?} at position: {position:?}");
                    return Ok(());
                }
    ...
    fn update_address_bar(&mut self) -> Result<(), Error> {
        // アドレスバーを白く塗りつぶす
        if self.window.fill_rect(WHITE, 72, 4, WINDOW_WIDTH - 76, ADDRESSBAR_HEIGHT - 2).is_err() {
            return Err(Error::InvalidUI(
                "failed to fill address bar rect".to_string(),
            ));
        }

        // input_urlをアドレスバーに描画する
        if self.window.draw_string(BLACK, 74, 6, &self.input_url, StringSize::Medium, false).is_err() {
            return Err(Error::InvalidUI(
                "failed to draw string to address bar".to_string(),
            ));
        }
    ...
    fn clear_address_bar(&mut self) -> Result<(), Error> {
        ...
        if self
            .window
            .fill_rect(WHITE, 72, 4, WINDOW_WIDTH - 76, ADDRESSBAR_HEIGHT - 2).is_err() {
            return Err(Error::InvalidUI(
                "failed to fill address bar rect".to_string(),
            ));
        }
